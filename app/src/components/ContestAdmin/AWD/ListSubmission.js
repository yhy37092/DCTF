...
export default ({contestId,filter}) => {
    ...
    return (
        <Table
            filter={filter}
            data={useCacheCall(['AWD', 'Moves', 'Teams', 'Challenges'], call => ((call('AWD', 'getContestSubmitIds', contestId) || []).flatMap(moveId => {
                const move = call('Moves', 'getMove', parseInt(moveId)) || defaultMove
                const team = call('Teams', 'getTeam', parseInt(move.teamId)) || defaultTeam
                const targetTeam = call('Teams', 'getTeam', parseInt(move.targetTeamId)) || defaultTeam
                ...
                return ({
                    ...
                    TargetTeamName: <TeamNamePreview team={targetTeam}/>,
